...
A = {
 ...
 'JPE.1':(550,440),'JPE.2':(590,440),'JPE.3':(630,440),'JPE.4':(670,440),'JPE.5':(710,440),
 'OUT.L':(1390,260),'OUT.N':(1390,350),'OUT.PE':(1390,440),'Fv.IN':(790,540),'Fv.OUT':(880,540),
 'D.L1':(1130,780),'D.L2':(1130,840),'D.N':(1130,900),'D.+':(1260,700),'D.-':(1320,700),'D.USB':(1550,1020),
 'D.DC+':(1130,1020),'D.DC-':(1130,1070),'CT.+':(1010,310),'CT.-':(1050,310),
 'PLATE':(290,1110),'RAIL':(460,1110),'AUX.L':(340,770),'AUX.N':(340,830),'AUX.PE':(340,890),
 'AUX.FACE.L':(570,790),'AUX.FACE.N':(570,850),'PSU.L':(710,790),'PSU.N':(710,850),
 'PSU.DC+':(890,810),'PSU.DC-':(890,860),'PC':(1660,1130),
}
# 01–19: installed conductors/cables; 20–23: existing factory plug/cable paths.
raw = [
 ...
 ('16','USB','D.USB','PC',[(1610,1020),(1610,1130)],(1610,1080),'USB Type B → protected exit → ELOG computer','signal'),
...
def validate():
    ...
    assert breakers=={'Q0'}
    assert [w['id'] for w in wires] == [f'{i:02}' for i in range(1,24)]
    for w in wires:
    ...
    for node in ['OUT.PE','AUX.PE','PLATE','RAIL']: assert closed(node)==closed('IN.PE')
    ...
    assert opened('IN.PE')==opened('OUT.PE')
...
def make_diagram():
    start_svg(1800,1270,'Single-breaker ELITEpro XC enclosure schematic, revision 3')
    ...
    for prefix,y,k,label in [('JL',260,'L','JL · HOT'),('JN',350,'N','JN · NEUTRAL'),('JPE',440,'PE','JPE · PE')]:
        rect(531,y-18,198,36,'#fff','#bbc7d0',6);line([(550,y),(710,y)],C[k],8);text(550,y-29,label,17,C[k],700)
    ...
    rect(1130,700,420,405,'#0a65a7','#084a78',12)
    text(1494,755,'DENT',34,'#fff',700,'end');text(1494,796,'ELITEpro XC',24,'#fff',600,'end');text(1494,830,'Functional port view',16,'#dfedfa',400,'end')
    for y,label in [(780,'L1'),(840,'L2'),(900,'N')]: text(1149,y+7,label,22,'#fff',700)
    for y,label in [(891,'L3: unused'),(925,'S: CT shield, not PE'),(959,'Analog inputs: unused')]:text(1380,y,label,18,'#fff',500,'middle')
    text(1260,729,'CH1 +',17,'#fff',600,'middle');text(1320,729,'CH1 −',17,'#fff',600,'middle')
    text(1150,1010,'POWER IN / OUT',18,'#fff',600);text(1150,1042,'Center +',17,'#fff');text(1150,1089,'Sleeve −',17,'#fff');text(1531,1053,'USB',17,'#fff',500,'end')
    ...
    text(456,970,'ELITEpro adapter only · UNMETERED',17,color='#8b651e',weight=600,anchor='middle')
...
def main():
    ...
    html=(OUT/'page_template.html').read_text().replace('<!-- MAIN_DIAGRAM -->',svg).replace('<!-- DETAIL_DIAGRAM -->',detail).replace('<!-- CONNECTION_ROWS -->',rows).replace('{{REV}}',REV)
