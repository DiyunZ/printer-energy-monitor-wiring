...
| [Front DXF](fabrication/front.dxf) · [Right DXF](fabrication/right.dxf) · [Left DXF](fabrication/left.dxf) · [Rear DXF](fabrication/rear.dxf) · [Panel DXF](fabrication/panel.dxf) | Millimetres, 1:1. Internal features are on `CUT`; outline/support locations still require the received enclosure and stock check. `REFERENCE` and `TEXT` are non-cutting. The right-wall file contains the USB service bore and two fixing holes. |
...
| USB service connection | Neutrik NAUSB-W, B outside / A inside; sleeved StarTech USB2HAB1 inside. Reuse the kit 1.8 m cable outside. | [DENT manual](https://www.dentinstruments.com/wp-content/uploads/EXC_ELOG19_11-15-24.pdf), printed pp. 16, 18, 29: USB power and insulation requirements. Export with mains unplugged and the lid closed. |
...
The original adapter cable stays entirely inside and plugs directly into the logger. Dress its slack using the reserved DC tie location; do not crush the flat jacket. Cable curves are illustrative: actual length, bend allowance, mains separation and lid clearance need dry fitting. No XA or DC wall opening remains. One USB service port is retained on the right wall.

USB configuration/download uses the side port with mains unplugged and verified absent. The lid stays closed. Remove the external PC cable before reconnecting mains; the short internal USB lead and its rated sleeve stay installed. Validate saved records with an offline download after a supervised pilot; the opaque cover hides the logger LEDs. Check adapter/enclosure temperature in that pilot.

### Closed-lid USB construction

Use the **Neutrik NAUSB-W** reversible insert as **USB-B outside / USB-A inside**. The internal **StarTech USB2HAB1** A-to-B lead is 1 ft (304.8 mm); the original kit cable stays outside. Route and restrain the internal lead beside the DC cable at the front/right support, keeping it away from mains wiring. The 3D curve is routing intent; store the actual remaining slack without tight bends or connector load.

The selected **black Alpha Wire F2213/4 BK105** sleeve is specified to **UL224 / CSA198, 600 V** ([manufacturer](https://www.alphawire.com/products/accessories/fit-heat-shrink-tubing/shrink-tubing/fit2213_4)). Buy one 4 ft stick. Minimum supplied ID is 19.05 mm; maximum recovered ID is 9.53 mm. Measure the actual plug molds before sleeving and verify complete internal-cable coverage, protected ends and retention against sliding. The rendered end boots are coverage illustrations made from that tubing allowance, not separately qualified parts.

Approve the tubing application with the cable/connector materials before assembly: Alpha lists 90°C minimum shrink and 121°C full recovery, while StarTech specifies a 60°C continuous operating limit. These describe different conditions and do **not** establish that an unreviewed heat process is acceptable. No arbitrary heating recipe or insulation qualification is supplied here. If the received assembly cannot be sleeved acceptably, resolve that interface before machining or energizing.

The port is a passive USB feedthrough, **not galvanic isolation**. Review exposed shell/shield treatment with the qualified assembler; do not create an undocumented PE-to-USB shield connection. Neutrik's IP65 claim requires its specified NKUSB mating cable and SCDP gasket. The ordinary kit cable and this modified enclosure have **no claimed IP/NEMA rating**; use the accepted assembly in its dry indoor environment.
...
| USB service | Ø24.00 mm project bore; two Ø3.20 mm project clearance holes, diagonal 19 × 24 mm pattern | Right wall; Y/Z = 55/167 mm. Outside local U/V offsets: −9.5/+12 and +9.5/−12 mm. [Neutrik drawing](https://www.neutrik.com/media/8974/download/nausb-w-2.pdf?v=1) requires bore ≥23.6 mm and nominal Ø3.1 fixings; check actual flange before cutting. |
...
| Three WAGO carriers + USB flange | 8 × M3 × 16 mm pan screws; 8 × M3 washers; 8 × M3 locknuts (6 carriers + 2 USB) | Buy 8 each, no additional M3 spares: [screws, 6835](https://boltdepot.com/Product-Details?product=6835), [washers, 4513](https://boltdepot.com/Product-Details?product=4513), [nuts, 4792](https://boltdepot.com/Product-Details?product=4792). |
...
Connections 01/03/06/07/10/11 use the supply/output cord conductors. Connections 05/08/09 use the three existing pigtail + full-lead chains; retain their full factory length. Connections 14/15 use the CT pair; 16 is the permanent sleeved internal lead plus the temporary external USB service cable, used with mains unplugged and the lid closed. Connections 20–23 illustrate factory adapter contacts and DC wiring, **not** instructions to open or modify the adapter. The site provides the full 21-path schedule; retained connection IDs stay unchanged.
...
4. Install Q0 and two power glands. Route intact supply/output jackets through the glands. Clamp the internal XA cord jacket in the 515CV body, fully seat and independently restrain the original adapter, and route its original DC cable inside. Fit the right-wall USB port, B out / A in, and the short internal cable with accepted 600 V sleeve/end protection. Remove only external PC USB before mains operation. XA/DC stay inside.
...
| Stock unavailable / designer | Reuse result ___; fallback decision ___ | A [Hammond 14R1513 panel](https://www.digikey.com/en/products/detail/hammond-manufacturing/14R1513/2359585), previously priced $30.44, is the documented fallback. Recheck its price and fit if needed; it is not included in the active $287.71 budget or a claim of aluminum reuse. |
...
| USB assembly / assembler | B-out/A-in orientation ___; cable mold and sleeve dimensions ___; approved sleeve process ___; end coverage and retention ___; separation/shield treatment ___ | DENT requires rated insulation for cable inside the electrical panel. Black FIT-221 is specified to UL224/CSA198, 600 V; the assembled cable has not been qualified. No PC connection during mains operation. |
| Offline configuration / operator | Actual CT model/range/output ___; ELOG configuration file ___; kit USB present ___ | Read the uncovered CT label. Do not assume 50 A from the photograph. Configure and download only with mains disconnected. |
