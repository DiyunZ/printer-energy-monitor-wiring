...
for (const [title, items] of [['✓ Equipment to reuse', [...owned,...kit]], ['□ Purchase and fabrication list', toBuy]]) {
 ...
 for (const p of items) text += `| ${p.availability === 'owned' ? '✓ Owned' : p.availability === 'kit' ? '✓ Kit included' : '□ To buy'} | <span class="material-name">${escape(p.item)}</span>${materialPhoto(p)}<a class="locate-material" data-material="${escape(p.id)}" href="index.html?material=${escape(p.id)}#layout" aria-label="View ${escape(p.item)} in 3D">View in 3D ↑</a> | ${p.quantity} | **${p.model}** — ${p.reason} | ${p.links.length ? p.links.map(l=>`[${l.label}](${l.url})`).join('<br>') : 'Reuse · No purchase needed'} | ${p.status} |\n`;
}
