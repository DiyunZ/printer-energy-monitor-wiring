...
(async () => {
  ...
  try {
    ...
    for (const p of bom.items) {
      ...
      assert.equal(await page.locator('#material-installation-note').textContent(),p.installation);
    ...
    const report={materials:bom.items.length,allLinksTested:true,uniqueInstalledMeshes:allMeshes.length,installedCounts:counts,individualLocationsTested:inspectedOccurrences,deepLinks:true,history:true,keyboard:true,viewportWidths:[1440,768,390],jsErrors:errors,physicalInstallationValidated:false};
