...
async function start() {
  ...
  // Source envelope follows Heyco 3104; locking details and draft are simplified.
  const up=instances['usb-entry'], ux=up.position[0]+up.size[0]/2-2, uy=up.position[1], uz=up.position[2];
  const bushStart=allMeshes.length;
  function usbRing(outer,depth,x){
    const shape=new THREE.Shape();shape.absarc(0,0,outer,0,Math.PI*2,false);
    const hole=new THREE.Path();hole.absarc(0,0,8.75,0,Math.PI*2,true);shape.holes.push(hole);
    ...
    return mesh(geo,material('#313c48',.5),[x,uy,uz],'entries');
  }
  usbRing(11.1,12.3,ux-6.15);usbRing(12.1,2,ux+1);
  planBodies['usb-entry']=allMeshes.slice(bushStart);
  mark('usb-bushing','bushing','USB cable exit',planBodies['usb-entry']);
  decal('USB · UNPLUG MAINS FIRST',55,7,[ux+1.5,uy+24,uz],'entries','right');
  tag('Existing USB cable · offline export',[ux+25,uy+30,uz],'entries');
  ...
  locator=installMaterialLocator({items:procurement.items,locations:materialLocations,scene,camera,controls,host,render,
    prepare:id=>{showDesignPanel('layout');resize();setAssembly(0,false);$('assembly-preview').hidden=true;$('assembly-preview').open=false;$('model-shell').value=['usb','usb-bushing'].includes(id)?'closed':'xray';$('model-wires').checked=true;appearance();selection.visible=false;},
    describe:p=>{
      activePart=p.id;
      materialCopy(p);
