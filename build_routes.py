...
html='''<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Single-printer monitor wiring</title><style>body{margin:0;background:#eef1f4;color:#172b40;font-family:"PingFang SC",sans-serif}header{padding:18px 24px;background:white;position:sticky;top:0;z-index:4;border-bottom:1px solid #cdd6df}h1{font-size:22px;margin:0 0 12px}.controls{display:flex;gap:8px;flex-wrap:wrap}button,a{font:inherit;font-size:14px;color:#20394f;background:#f3f6f9;border:1px solid #c7d1dc;padding:8px 12px;border-radius:6px;cursor:pointer;text-decoration:none}button.active{background:#173f64;color:white}#status{font-size:14px;margin:12px 0 0;color:#586e83}.stage{overflow:auto;padding:14px}.stage svg{display:block;width:100%;min-width:1100px;height:auto;background:white}footer{padding:18px 28px;font-size:14px;line-height:1.8;background:#fff}footer a{padding:0;border:0;color:#176ba7}@media print{header,.stage+footer{display:none}.stage{padding:0}.stage svg{min-width:0;width:100%}@page{size:A3 landscape;margin:6mm}}</style><header><h1>Single-printer monitor | Trace each wire</h1><div class="controls"><button data-mode="all" class="active">All wires</button><button data-mode="main">1. Printer line</button><button data-mode="voltage">2. DENT line branch</button><button data-mode="neutral">3. Neutral</button><button data-mode="earth">4. Protective earth</button><button data-mode="signal">5. CT / USB</button><button id="zoom">Zoom 150%</button><a href="wiring_routes.png" download>Download PNG</a><a href="wiring_routes.svg" download>Download SVG</a><a href="references.html">References / evidence</a></div><p id="status">Click a numbered row to trace one wire, or select a circuit group above.</p></header><div class="stage">'''+svg+'''</div><footer>Candidate routing plan, not an approved construction drawing. Based on <a href="https://www.dentinstruments.com/wp-content/uploads/EXC_ELOG19_11-15-24.pdf">DENT manual (single-phase diagram: printed page 55)</a> and the existing 16-connection project list. Moving the DENT branch upstream of CT is a project adaptation pending review.<br>Purple is a diagram color, not a specified CT wire color. Verify sensor model and polarity. Port positions, spacing and bend radii are schematic. Disconnect mains and USB and verify absence of voltage before opening.</footer><script>
...
let zoom=false;document.getElementById('zoom').onclick=()=>{zoom=!zoom;document.querySelector('.stage svg').style.width=zoom?'150%':'100%';document.getElementById('zoom').textContent=zoom?'Reset zoom':'Zoom 150%'};
</script></html>'''
