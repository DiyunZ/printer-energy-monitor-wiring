...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#main-stage .wire-route').count(), 25);
    assert.equal(await page.locator('#main-stage .wire-badge:visible').count(), 0, 'Default hides wire numbers');
    assert.equal(await page.locator('#main-stage text:visible').count(), 24, 'Overview keeps short labels only');
    ...
    await page.locator('#wire-25').focus(); await page.keyboard.press('Enter');
    assert.equal(await page.locator('#trace-endpoints').textContent(), 'PE 1 port 5 → PE 2 port 5');
    ...
    const report = { defaultVisibleLabels: 24, allPaths: 25, defaultLabelMinimumPx: Math.min(...labels), fitAll: true, focusWireAndGroup: true, physicalEndpointNames: true, dragAndKeyboardPan: true, fullscreenAndEscape: true, viewportWidths: [1440,768,390,320], errors };
